...
locals {
  s3_static_origin_id = "diceytech-${var.site}-static-origin"
  s3_media_origin_id = "diceytech-${var.site}-media-origin"
}
...
resource "aws_cloudfront_origin_access_identity" "richie_oai" {
  comment = "diceytech ${var.site} origin for the ${terraform.workspace} environment"
}
